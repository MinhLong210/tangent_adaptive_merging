...
#                         # Compute logits as in your implementation
#                         with torch.no_grad():
...
#                         def model_forward(param, inputs, text_embeds, batch_indices):
...
#                             # Use functional_call to compute the forward pass
...
#                             # Access visual_projection from the base model if clip_model is a PeftModel
#                             if hasattr(clip_model, 'base_model'):
...
#                             return logits_per_image[batch_indices]  # Shape: (len(batch_indices), num_classes)

#                         # Compute the Jacobian in chunks (e.g., 4 samples at a time)
#                         from torch.func import jacrev
#                         chunk_size = 8  # Adjust based on memory constraints
#                         grads_list = []
#                         for b_start in range(0, batch_size, chunk_size):
#                             b_end = min(b_start + chunk_size, batch_size)
#                             batch_indices = torch.arange(b_start, b_end).to(images.device)

#                             jacobian_fn = jacrev(model_forward, argnums=0)
#                             jacobian = jacobian_fn(param, inputs, text_embeds, batch_indices)  # Shape: (chunk_size, output_dim, param_shape)

#                             # Reshape and slice the Jacobian
#                             chunk_size_actual = b_end - b_start
#                             jacobian_slice = jacobian[:, :, slice_start:slice_end,:].reshape(chunk_size_actual * output_dim, -1)  # Shape: (chunk_size * output_dim, slice_param_size)
#                             grads_list.append(jacobian_slice.detach())

#                             del jacobian, jacobian_slice
#                             torch.cuda.empty_cache()

#                         A_matrix_slice = torch.cat(grads_list, dim=0).to(images.device)  # Shape: (batch_size * output_dim, slice_param_size)
#                         b_vector = (logits - labels).flatten()
#                         global_At_A.add_(A_matrix_slice.T @ A_matrix_slice)
#                         global_At_b.add_(A_matrix_slice.T @ b_vector)

#                         del A_matrix_slice, b_vector, logits, grads_list
